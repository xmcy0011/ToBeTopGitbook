...
PS：网上比较好的教程是*丁奇 2018年《MySQL 实战 45 讲》*，经确认，MySQL 超过 8.0.13 版本后，其中部分 Case 失效（案例三：主键索引范围锁）。生产环境中使用 MySQL 5.7 比较多，但是 8.0.13 中的锁分析功能更好用。故为了更好的理解和分析间隙锁，本文结果输出使用 MySQL 8.0.13，经初步验证，其加锁表现和 MySQL 5.7 以及 MariaDB 10.4.33一致。
...
- 等值查询
  ...
  - 非唯一索引（**普通索引**，即二级索引）
    - 记录存在：因为是普通索引，可以插入重复的值，故需要同时锁定记录本身、记录之前的间隙和记录之后的间隙。又因为加锁单位是临键锁，故最终会加2个临键锁。同时，对该值对应的主键记录加记录锁，即加3把锁，参考case9
    - 记录不存在：*临键锁退化成间隙锁*，参考case10
  - 覆盖索引（**lock in share mode**，如果是加独占锁，规则同非唯一索引）
    - 记录存在：只加覆盖索引的记录锁，不涉及回表查询，故无需访问主键数据，参考case13
    - 记录不存在：同上，退化成普通索引的间隙锁，没有记录，所以也不会有主键索引加间隙锁一说，参考case14
- 范围查询
  - 聚簇索引：对所有符合查询条件的记录和记录之前的间隙加锁，即 Next-Key Locks，如果是 ">=" 或 "<=" 该记录存在，则只对该记录加记录锁，记录之前的间隙不加锁，锁是加在主键索引上，参考case11
  - 二级索引：对所有符合查询条件的记录和记录之前的间隙加锁，即 Next-Key Locks，同时对命中二级索引对应的主键索引加记录锁。即临键锁加在二级索引上，记录锁加在主键索引上，参考case9
...
> PS: 请确认隔离级别是可重复读（Repeatable Read），间隙锁只在该级别下使用。
...
|   | | update test set c=c+1 where id=15;<p style="color:green">(OK)</p> |
|  commit | | |

根据原则1，加锁的基本单位是 Next-Key Locks，事务A虽然是 update，但也会首先对 (10,15] 加锁，但是因为记录不存在，故退化为间隙锁(10,15)。事务B向这个范围插入被锁住，事务C可以执行。
...
||insert into test values(8,8,8);<p style="color:green">(OK)</p>insert into test values(13,13,13);<p style="color:red">(Blocked)</p>||
|||update set d=d+1 where id=15;<p style="color:red">(Blocked)</p>|
...
- 但是需要先找到 id=10 的行，根据优化 1，主键 id 上的等值条件，退化成行锁，故只对 id=10 这一行加锁。
...
|   | insert into test values(16,16,16);<p style="color:red">(Blocked)</p> | | |
|   | | update test set c=c+1 where id=15;<p style="color:red">(Blocked)</p> |
|   | | | update test set c=c+1 where id=20;<p style="color:green">(OK)</p> |
|  commit | | | |
...
此时事务A加的锁为c索引的 (10,15] 和 (15,20)，以及 id=15 的主键记录锁。事务B所在区间被锁住，所以会阻塞。事务C因为主键 id=15 已被加锁，故也会阻塞，事务D可以执行，因为事务A只对 (15,20) 加了间隙锁。
...
|   | insert into test values(13,13,13);<p style="color:red">(Blocked)</p> | |
|   | | update test set d=d+1 where id=5;<p style="color:green">(OK)</p> |
|  commit | | |
...
所以，事务B插入会阻塞，而事务C成功。如若使用 lock for update 独占锁(X)，系统会认为你接下来要更新数据，所以会对主键索引上满足条件的行加锁，故事务C会被阻塞，无法更新。
...
||insert into test values(8,8,8);<p style="color:red">(Blocked)</p>||
|||update set d=d+1 where id=15;<p style="color:red">(Blocked)</p>|
