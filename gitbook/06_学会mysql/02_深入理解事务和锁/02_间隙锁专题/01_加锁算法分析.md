...
间隙锁的出现，是为了阻止记录的插入，在读已提交级别下，只有记录锁。记录锁只能阻塞其他事务更新或者删除数据，无法阻止插入。这也是幻读问题的根源，所以在可重复读级别下，引入了间隙锁和插入意向锁。

既然间隙锁只是针对插入语句，那么当前读语句中的间隙锁之间不冲突，但是插入语句会申请特殊的间隙锁，即插入意向锁（Insert Intention Locks），插入意向锁和当前读语句中的间隙锁是冲突的，这样才能阻止幻读问题。
> PS: 当前读语句 select for update & lock in share mode,update,delete

这也是为什么，在“存在返回，不存在插入”的业务场景中，可重复读级别下会出现死锁的根本原因。

有如下事务（9和6都不存在）：

| 步骤 | 事务A | 事务B |
|-|-|-|
| 1 | begin;<br />select * from test where id=9 for update; | |
| 2 | | begin;<br />select * from test where id=6 for update;|
| 3 | | insert into test values(7,7,7);<br /><font color="red">blocked;</font>
| 4 | insert into test values(7,7,7);<br /><font color="red">ERROR 1213 (40001): Deadlock found when trying to get lock; try restarting transaction</font> | |

- 因为记录不存在，故事务A和事务B，在步骤1和步骤2都成功获取了同一个 (5,10) 的间隙锁（X,GAP）
- 在步骤3，事务B需要加插入意向锁（X,GAP,INSERT_INTENTION） (5,10)，因为该间隙已被事务 A 获取，故此时阻塞
-  步骤4，事务A执行插入语句，获取插入意向锁，很明显，需要事务B的间隙锁释放才能获取到

至此，双方互相等待对方的间隙锁释放，双双阻塞，造成死锁。
...
在可重复的（Repeatable Read）级别下，加锁的基本单位是 Next-Key Locks，不管是 select、update 还是 delete 语句，都会优先尝试加 Next-Key Locks。

如果是 insert 操作，则会优先加表级别的插入意向锁（Insert Intention Locks）
...
如上，所以事务A 执行 update 操作，会首先加 (10,15] 的 Next-Key Locks 锁，但是因为记录不存在，故退化为间隙锁(10,15)。事务B向这个范围插入被锁住，事务C可以执行。
