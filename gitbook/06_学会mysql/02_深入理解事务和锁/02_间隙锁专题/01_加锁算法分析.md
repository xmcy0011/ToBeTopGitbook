...
根据原则1，加锁的基本单位是 Next-Key Locks，不管是 select、update 还是 delete 语句，所以事务A 执行 update 操作，会首先加 (10,15] 的 Next-Key Locks 锁，但是因为记录不存在，故退化为间隙锁(10,15)。事务B向这个范围插入被锁住，事务C可以执行。
...
|   | insert into test values(16,16,16);<p style="color:red">(Blocked)</p> | | |
|   | | update test set c=c+1 where id=15;<p style="color:red">(Blocked)</p> |
|   | | | update test set c=c+1 where id=20;<p style="color:red">(Blocked)</p> |
|  commit | | | |
